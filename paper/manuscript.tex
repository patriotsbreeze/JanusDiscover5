...
SMIRNOFF force field.  We demonstrate the platform on three
clinically relevant kinase targets---ABL1, EGFR, and CDK2---achieving
AUC-ROC values of
\placeholder{0.XX}, \placeholder{0.XX}, and \placeholder{0.XX},
BEDROC values of
\placeholder{0.XX}, \placeholder{0.XX}, and \placeholder{0.XX},
and redocking RMSD success rates of
\placeholder{XX}\%, \placeholder{XX}\%, and \placeholder{XX}\%,
respectively.  JanusDiscover is freely available at
...
\leq \SI{1}{\micro\molar}$) restricted to binding assays
...
\begin{enumerate}
  ...
  \item NPT equilibration: \SI{100}{\pico\second} at \SI{1}{\atmosphere},
...
%% ── 3.1 ─────────────────────────────────────────────────────────────────────
\subsection{Case Studies: ABL1, EGFR, and CDK2}

We selected three well-characterised kinase targets to demonstrate the
end-to-end JanusDiscover workflow.  These targets span a range of available
structural data (ABL1: \placeholder{N} PDB entries;
EGFR: \placeholder{N}; CDK2: \placeholder{N}) and clinical contexts
(chronic myeloid leukaemia, non-small-cell lung carcinoma, and cell-cycle
dysregulation, respectively).  For each target, the AI literature review
correctly identified the approved first-line therapy and recommended
SBDD or hybrid mode based on the abundance of crystal structures.

Training sets comprised \placeholder{N}, \placeholder{N}, and \placeholder{N}
known binding-assay actives (pChEMBL $\geq 6.0$) from ChEMBL for ABL1,
EGFR, and CDK2 respectively.  The ZINC-250k library was used as the
screening dataset for all three targets.  Property-matched decoys were
generated at a 5:1 decoy-to-active ratio as described in
Section~2.3.
...
%% ── 3.2 ─────────────────────────────────────────────────────────────────────
\subsection{LBDD Performance}

Table~\ref{tab:lbdd} reports the LBDD validation metrics for all three
targets.  AUC-ROC values of \placeholder{0.XX}, \placeholder{0.XX}, and
\placeholder{0.XX} were obtained for ABL1, EGFR, and CDK2 respectively,
consistent with the typical range of 0.75--0.92 reported for Random Forest
ECFP4 classifiers on kinase datasets.\cite{svetnik2003random}
BEDROC values of \placeholder{0.XX} (ABL1), \placeholder{0.XX} (EGFR),
and \placeholder{0.XX} (CDK2) confirm early-enrichment performance
substantially above the random-ranking baseline of $\approx 0.12$ for a
10\% active rate at $\alpha = 20$.

Early-enrichment factors were
EF$_{1\%} = \placeholder{X.X}\times$ and
EF$_{5\%} = \placeholder{X.X}\times$ for ABL1,
reflecting the structural diversity of known ABL1 inhibitors captured by
the ECFP4 representation.  ROC curves and enrichment curves for all three
targets are provided in Figures S1--S3 of the Supporting Information.

\begin{table}[htbp]
  \centering
  \caption{LBDD validation metrics for three kinase targets.
           All values are obtained by five-fold stratified cross-validation.
           BEDROC 95\% CI estimated by 1000 bootstrap resamples.}
  \label{tab:lbdd}
  \begin{tabular}{lcccc}
    \toprule
    Target &
    ...
    ABL1 &
      \placeholder{0.XX} &
      \placeholder{0.XX} (\placeholder{0.XX}--\placeholder{0.XX}) &
      \placeholder{X.X}$\times$ &
      \placeholder{X.X}$\times$ \\
    EGFR &
      \placeholder{0.XX} &
      \placeholder{0.XX} (\placeholder{0.XX}--\placeholder{0.XX}) &
...
For structure-based screening, the highest-resolution PDB entries for each
target were selected: \placeholder{PDBID} (\placeholder{X.XX}\,\AA{}) for
ABL1, \placeholder{PDBID} (\placeholder{X.XX}\,\AA{}) for EGFR, and
\placeholder{PDBID} (\placeholder{X.XX}\,\AA{}) for CDK2.  In all three
cases, JanusDiscover correctly identified the co-crystallised ligand from
HETATM records and placed the docking box at its centroid.

Redocking of the native co-crystal ligand produced RMSD values of
\placeholder{X.XX}\,\AA{} (ABL1), \placeholder{X.XX}\,\AA{} (EGFR), and
\placeholder{X.XX}\,\AA{} (CDK2), all below the canonical success threshold
of \SI{2.0}{\angstrom} (Table~\ref{tab:sbdd}).  The docking score
distributions separated active and inactive compounds
(Figure~\ref{fig:sbdd}a--c), yielding AUC-ROC values of
\placeholder{0.XX} (ABL1), \placeholder{0.XX} (EGFR), and
\placeholder{0.XX} (CDK2).

\begin{table}[htbp]
  \centering
  \caption{SBDD redocking validation and enrichment metrics.}
  \label{tab:sbdd}
  \begin{tabular}{lcccccc}
    ...
    \midrule
    ABL1 &
      \placeholder{XXXX} & \placeholder{X.X} &
      \placeholder{X.XX} & \placeholder{0.XX} &
      \placeholder{X.X}$\times$ & \placeholder{X.X}$\times$ \\
    EGFR &
      \placeholder{XXXX} & \placeholder{X.X} &
      \placeholder{X.XX} & \placeholder{0.XX} &
      \placeholder{X.X}$\times$ & \placeholder{X.X}$\times$ \\
...
Applied to three clinically relevant kinase targets---ABL1, EGFR, and
CDK2---JanusDiscover returned AUC-ROC values of \placeholder{$\geq 0.80$},
BEDROC values of \placeholder{$\geq 0.60$}, and redocking RMSD values below
\SI{2.0}{\angstrom} for all targets, demonstrating that automated pipeline
integration does not sacrifice scientific rigour.
...
\begin{figure}[htbp]
  ...
  \caption{%
    LBDD virtual screening results for ABL1 kinase.
    \textbf{(a)}~ROC curve (five-fold cross-validation); shaded band shows
    $\pm 1\,\sigma$ over folds.
    \textbf{(b)}~Enrichment curve; horizontal dashed line indicates random
    baseline (EF~$= 1$).
    \textbf{(c)}~Score distributions for actives (blue) and decoys (red).
    \textbf{(d)}~Metrics summary bar chart (AUC-ROC, BEDROC, EF$_{1\%}$,
    EF$_{5\%}$).%
  }
  \label{fig:lbdd}
\end{figure}
