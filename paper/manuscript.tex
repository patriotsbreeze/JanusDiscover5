...
%% ── Author block ────────────────────────────────────────────────────────────
\author{Brendan Lo}
\affiliation{Department of Chemistry, University of Chicago,
...
substantial scripting expertise (KNIME, HTMD).\cite{doerr2017htmd}  This fragmentation forces researchers to manually transfer
...
The protein structure is processed with PDBFixer~1.12:\cite{eastman2024openmm8}
non-standard residues and solvent molecules are removed, missing residues
($\leq 10$ consecutive) are modelled, and hydrogens are added at pH~7.4.
The protein is parameterised with AMBER\,ff14SB\cite{maier2015ff14sb} using
OpenMM~8.5.\cite{eastman2024openmm8}  For the current benchmark the
receptor chain~A is simulated without the co-crystal ligand
(protein-only protocol) because the OpenFF SMIRNOFF Sage force field and
GAFF2 template generators require the OpenFF Toolkit,\cite{qiu2021forcefield}
which is not yet available as a pre-built wheel for Python~3.12 on Windows.
The system is solvated in a rectangular TIP3P\cite{jorgensen1983comparison}
water box with \SI{1.0}{\nano\meter} padding and 0.15\,M NaCl.

\paragraph{MD protocol.}
The simulation protocol follows established best practice:\cite{maier2015ff14sb}
\begin{enumerate}
  \item Energy minimisation (steepest descent, 1000 steps maximum).
  \item NVT equilibration: \SI{500}{\pico\second} at \SI{300}{\kelvin},
        LangevinMiddle thermostat with $\gamma = \SI{1}{\per\pico\second}$,
        \SI{2}{\femto\second} time step, hydrogen-bond constraints (HBonds).
  \item NPT equilibration: \SI{500}{\pico\second} at 1\,atm,
        Monte Carlo barostat, same thermostat.
  \item Production run of user-specified duration (default \SI{10}{\nano\second});
        GPU acceleration via OpenCL or CUDA is used automatically when available.
\end{enumerate}

A DCD trajectory is written every 10\,ps (5\,000 steps).

\paragraph{Trajectory analysis.}
Post-simulation analysis is performed with MDTraj~1.11.\cite{eastman2017openmm}
The following observables are computed per frame: (i)~C$\alpha$ RMSD relative
to the first production frame; (ii)~per-residue C$\alpha$ RMSF;
(iii)~radius of gyration ($R_g$); (iv)~potential energy from the
OpenMM \texttt{StateDataReporter}; (v)~total solvent-accessible surface area
(SASA) using the Shrake--Rupley algorithm\cite{shrake1973environment}
implemented in MDTraj; (vi)~binding-site C$\alpha$ RMSD for residues within
5\,\AA{} of the co-crystal ligand centroid.
...
(84 PDB structures in UniProt release 2024\_05 for human ABL1, UniProt P00519),
a well-characterised target in
...
ranking identified 5 compounds in common, with no compounds appearing
exclusively in the hybrid top-20.  The top-ranked hybrid hit
(\texttt{Cc1ccc(NC(=O)Cc2nc(-c3cccc(Br)c3)cs2)cc1}) achieved a predicted
active probability of 0.092 (LBDD) and a docking score of
$-10.38$\,kcal/mol (SBDD).  Scaffold analysis using the Bemis--Murcko
framework\cite{bemis1996properties} confirmed chemical diversity across the
top-20 list, with 20 distinct scaffolds represented.

%% ── 3.5 ─────────────────────────────────────────────────────────────────────
\subsection{Molecular Dynamics Simulation of the ABL1 Binding Site}

The ABL1 kinase domain (chain A, PDB 2HYY, 4\,402 protein atoms) was
subjected to a \SI{10}{\nano\second} all-atom explicit-solvent MD simulation
using OpenMM~8.5 with AMBER\,ff14SB and TIP3P water (70\,049 total atoms;
NVIDIA GeForce GTX~1080, OpenCL mixed precision).  Following NVT and NPT
equilibration (500~ps each), the production trajectory (1\,000 frames at
10~ps intervals) was analysed with MDTraj~1.11.

Figure~\ref{fig:md}a shows the C$\alpha$ RMSD trajectory.  After an initial
relaxation of $\approx \SI{1}{\nano\second}$, the backbone stabilised at a
mean RMSD of $1.76 \pm 0.27$\,\AA{} (full production phase), with a maximum
of 2.51\,\AA{}, consistent with a well-equilibrated kinase domain.

Per-residue RMSF analysis (Figure~\ref{fig:md}b) identified the
\textit{P-loop} (residues 248--259) and the \textit{activation loop}
(residues 380--385) as the most flexible segments, consistent with
published crystallographic $B$-factor data for ABL1.\cite{nagar2002structural}
The binding-site C$\alpha$ RMSD (residues within 5\,\AA{} of co-crystal
imatinib) remained exceptionally stable at $0.76 \pm 0.20$\,\AA{}
(Figure~\ref{fig:md}f), confirming that the active-site geometry is
preserved throughout the simulation.

The radius of gyration remained stable at $19.44 \pm 0.11$\,\AA{}
(Figure~\ref{fig:md}c), confirming global structural integrity.  The
potential energy equilibrated within the first \SI{500}{\pico\second} of
production (Figure~\ref{fig:md}d), and the solvent-accessible surface area
oscillated around $14{,}215$\,\AA{}$^2$ (Figure~\ref{fig:md}e), both
indicators of a well-equilibrated system.

Together, these results demonstrate that the ABL1 binding site retains its
active conformation throughout the simulation, validating the structural
basis of the SBDD virtual screening hits.

%% ── 3.6 ─────────────────────────────────────────────────────────────────────
\subsection{Computational Performance}

All experiments were performed on a desktop workstation running Windows~10
with an Intel Core i7-7700K CPU (4.2\,GHz, 4 cores), 32\,GB DDR4 RAM,
and an NVIDIA GeForce GTX~1080 (8\,GB VRAM).
Table~\ref{tab:timing} reports wall-clock times for each pipeline stage.

\begin{table}[htbp]
  \centering
  \caption{Approximate wall-clock times for each JanusDiscover pipeline
           stage on an Intel Core i7 workstation with NVIDIA GeForce GTX~1080 (8\,GB).
           Times are per target.}
  \label{tab:timing}
  \begin{tabular}{lc}
    \toprule
    Stage & Wall time \\
...
teams for providing freely available data and software.  No external funding was received for this work.
\end{acknowledgement}
...
\begin{figure}[htbp]
  ...
  \caption{%
    Molecular dynamics simulation of ABL1 kinase (PDB 2HYY chain~A,
    \SI{10}{\nano\second}, AMBER ff14SB + TIP3P, 70\,049 atoms,
    NVIDIA GTX~1080 OpenCL).
    \textbf{(a)}~C$\alpha$ RMSD vs.\ time (mean $1.76$\,\AA, max $2.51$\,\AA).
    \textbf{(b)}~Per-residue C$\alpha$ RMSF; P-loop (248--259) and
    activation loop (380--385) are the most flexible regions.
    \textbf{(c)}~Radius of gyration vs.\ time (mean $19.44 \pm 0.11$\,\AA).
    \textbf{(d)}~Potential energy vs.\ time (kJ~mol$^{-1}$).
    \textbf{(e)}~Solvent-accessible surface area vs.\ time
    (mean $14{,}215$\,\AA$^{2}$).
    \textbf{(f)}~Binding-site C$\alpha$ RMSD (residues within 5\,\AA{}
    of co-crystal imatinib; mean $0.76$\,\AA), confirming active-site
